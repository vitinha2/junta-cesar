// CONFIGURAÇÃO DO FIREBASE (Chaves do teu projeto)
...
function carregarDados() {
    // O '.on' faz com que o site se atualize sozinho sempre que houver mudanças na nuvem
    db.ref('reservas').on('value', (snapshot) => {
        const dados = snapshot.val();
        reservasAtivas = [];
        
        if (dados) {
            // Converte o objeto do Firebase para o formato de array que o teu código já usava
        ...
        if (document.getElementById('vistaHistorico').style.display === 'block') {
            renderizarHistorico();
        }
    });
}

// O carregarDados agora é chamado logo no início para estar pronto após o login
carregarDados();
...
function abrirForm(data, equip, idFirebase = -1) {
    ...
    document.getElementById('editIndex').value = idFirebase; // Agora guarda o ID da nuvem
...
document.getElementById('reservaForm').addEventListener('submit', function(e) {
    ...
    if (idFirebase === "-1") {
        // Criar nova reserva na nuvem
        db.ref('reservas').push(dados);
    } else {
        // Atualizar reserva existente na nuvem
    ...
    document.getElementById('formularioReserva').style.display = 'none';
    verificarDisponibilidade();
    if (idFirebase !== "-1") alternarVista('historico');
});
...
function renderizarHistorico() {
    ...
    // Ordenar por data (opcional, para ficar organizado)
    const ordenadas = [...reservasAtivas].sort((a, b) => new Date(a.data) - new Date(b.data));
...
function apagar(idFirebase) {
    if(confirm("Deseja eliminar esta reserva permanentemente para todos os utilizadores?")) { 
